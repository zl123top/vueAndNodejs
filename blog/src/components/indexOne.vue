...
<div class="join">
     ...
     <input type="tel" placeholder="手机号" class="number" v-model="phon" @focus="focusOne">
     <input type="tel" placeholder="输入验证码" class="auth" v-model="pass" @change="oneSelf(pass)" @focus="focusOne">
...
export default {
  ...
  methods: {
    // 两秒获取
    timeTest(){
      if(this.$store.state.ws.readyState!=0)
      setTimeout(()=>{
          this.reconnect()
      },2000)
    },
      ...
      if (this.phon&&this.pass) {
        // 手机验证码登录 获取用户信息
              ...
              ws.onopen = () => {
                // alert(1)
                ws.send('{"action":"LoginWithMobileVerifyCode",'+phonOne+','+passOne+'}');
                setTimeout(function(){console.log(ws.readyState)},10000)

              };
              ws.onmessage =this.firstLogin
              ws.onerror=()=>{
                console.log('连接发生错误！等待重连')
                this.timeTest()
              }  
              ws.onclose=()=>{
                console.log('断开了')
